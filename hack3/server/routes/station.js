...
const tidyUpData = (data, result) => {
  ...
  data.map(function(obj) {
    if (!result[obj.station_type]) result[obj.station_type] = []
    result[obj.station_type].push(obj)
    result[obj.station_type].sort(function(a, b) {
...
const CalculateDistance = async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(result).length) {
      ...
      res.status(200).send({
        message: 'success',
        distance: answer // the distance after calculating
      })
    }
    else {
      throw new Error('Something Wrong !')
